...
<script type="text/javascript">
  ...
  function tambahdata(){
      $.ajax({
                    url: "{{config('app.url')}}/linmas_tibum/store",
                    method: "POST",
                    headers: {
 ...
 function deletedata(objectid){
          Swal.fire({
                ...
                if (result.isConfirmed) {
                      $.ajax({
                            url: "{{config('app.url')}}/linmas_tibum/delete/"+objectid,
                            type: "GET",
  ...
  function getData(objectid){
    $("#modaledit").modal();
    $.ajax({
        url: "{{config('app.url')}}/linmas_tibum/getdata",
        type: "GET",
        data: {objectid: objectid},
        headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
     ...
     headers: {
      ...
      success:function(res){
        if(res.status == "success"){
          ...
          let jenisgangguan = $('#jenigangguan_edit').val(res.data.jenisgangguan);
...
$('#modaltambah').on('show.bs.modal', function(){
 ...
 function updateDataAjax(){
  ...
  $.ajax({
      url: "{{config('app.url')}}/linmas_tibum/update",
      method: "POST",
      headers: {
          'X-CSRF-TOKEN': '{{ csrf_token() }}'
      },
